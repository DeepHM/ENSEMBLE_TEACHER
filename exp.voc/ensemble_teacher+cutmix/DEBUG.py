...
# from dataloader_org import get_train_loader
from dataloader_aug import get_train_loader

from network import Network
# from dataloader_org import VOC
from dataloader_aug import VOC
...
#                 mode='fan_in', nonlinearity='relu')
    ...
    model_file = os.path.join(config.volna,'output_path/voc8_res50_CPScutmix_B8E300_ORIGIN_Usetepoch/snapshot/epoch-54.pth')
    ckpt = torch.load(model_file)['model']
...
#         pbar = tqdm(range(config.niters_per_epoch), file=sys.stdout, bar_format=bar_format)
        ...
        for idx in pbar:
            ...
            if (epoch <= 3) and (idx == 0) :
                print(engine.local_rank,'unsup_minibatch_0',unsup_minibatch_0['fn'])
                print(engine.local_rank,'unsup_minibatch_1',unsup_minibatch_1['fn'])
            ...
            print('unsup_imgs_strong_0.shape',unsup_imgs_strong_0.shape)
...
#             unsup_imgs_mixed = unsup_imgs_0 * (1 - batch_mix_masks) + unsup_imgs_1 * batch_mix_masks
            ...
            check1 = unsup_imgs_0.cpu().numpy()
            check2 = unsup_imgs_1.cpu().numpy()
            check3 = unsup_imgs_mixed.cpu().numpy()
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_unsup_imgs_0',check1)
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_unsup_imgs_1',check2)
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_unsup_imgs_mixed',check3)
            check4 = logits_cons_stu_1.cpu().detach().numpy()
            check5 = logits_cons_stu_2.cpu().detach().numpy()
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check4',check4)
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check5',check5)
            check6 = unsup_imgs_strong_0.cpu().detach().numpy()
            check7 = unsup_imgs_strong_1.cpu().detach().numpy()
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check6',check6)
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check7',check7)
            check8 = ps_label_1.cpu().detach().numpy()
            check9 = ps_label_2.cpu().detach().numpy()
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check8',check8)
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check9',check9)
            check10 = batch_mix_masks.cpu().detach().numpy()
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check10',check10)
            check11 = batch_mix_masks.cpu().detach().numpy()
            np.save(f'CHECK_FILES/Debug_Rank{engine.local_rank}_check11',check11)            
