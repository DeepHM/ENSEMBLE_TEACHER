...
#         rand_aug = Randaug(Numbers=3, Magnitude=20, max_Magnitude=40,p=1.0)
        ...
        return p_img, p_img_strong, p_gt, extra_dict
...
def get_train_loader(engine, dataset, train_source, unsupervised=False, collate_fn=None, sampler_seed = None):
...
#             img, gt, extra_dict = self.preprocess(img, gt)
            img, img_strong, gt, extra_dict = self.preprocess(img, gt)
        ...
        output_dict = dict(data=img, data_strong=img_strong , fn=str(names),
                           n=len(self._file_names))
        if gt is not None:
            extra_dict['label'] = gt
