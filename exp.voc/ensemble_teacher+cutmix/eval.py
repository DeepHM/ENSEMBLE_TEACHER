...
from dataloader_org import ValPre
...
if __name__ == "__main__":  
    ...
    parser.add_argument('-ens', '--ensemble', default='v0',choices=['v0','v0_2','v1','v1_2','v2','v3','v4'], type=str)
    ...
    if not os.path.exists('log_df.csv') :
        log_df = pd.DataFrame(columns=['log_file','folder','model_file','mean_IU','mean_IU_no_back','mean_pixel_ACC','Ensemble'])
        log_df.to_csv('log_df.csv',index=False)

#     network = Network(config.num_classes, criterion=None, norm_layer=nn.BatchNorm2d)
    network = Network(config.num_classes, criterion=None, norm_layer=nn.BatchNorm2d, pretrained_model=config.pretrained_model)
    ...
    config.val_log_file = config.val_log_file.split('/')[-1].split('.')[0] + '_ensemble' + args.ensemble.upper() + '.log'
    config.val_log_file = osp.join(config.log_dir,config.val_log_file)

    with torch.no_grad():        
        segmentor = SegEvaluator(dataset, config.num_classes, config.image_mean,
                         config.image_std, network,
                         config.eval_scale_array, config.eval_flip,
                         all_dev, args.verbose, args.save_path, args.show_image,
                         ensemble=args.ensemble, ema_model_path=config.ema_snapshot_dir)
