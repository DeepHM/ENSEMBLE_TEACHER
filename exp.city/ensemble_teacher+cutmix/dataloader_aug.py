...
from augmentation.transform import Randaug


def random_mirror(img, strong_img, gt=None):
    if random.random() >= 0.5:
        img = cv2.flip(img, 1)
        strong_img = cv2.flip(strong_img, 1)
        if gt is not None:
            gt = cv2.flip(gt, 1)

    return img, strong_img, gt
...
def random_scale(img, strong_img, gt=None, scales=None):
    scale = random.choice(scales)     # scale = random.uniform(scales[0], scales[-1])
    sh = int(img.shape[0] * scale)
    sw = int(img.shape[1] * scale)
    img = cv2.resize(img, (sw, sh), interpolation=cv2.INTER_LINEAR)
    strong_img = cv2.resize(strong_img, (sw, sh), interpolation=cv2.INTER_LINEAR)
    if gt is not None:
        gt = cv2.resize(gt, (sw, sh), interpolation=cv2.INTER_NEAREST)

    return img, strong_img, gt, scale
...
class TrainPre(object):
    ...
    def __call__(self, img, gt=None):
        ...
        rand_aug = Randaug(Numbers=3, Magnitude=20, max_Magnitude=40,p=0.5)
        
        # gt = gt - 1     # label 0 is invalid, this operation transfers label 0 to label 255
        
        strong_img = copy.deepcopy(img) 
        img, strong_img, gt = random_mirror(img, strong_img, gt)
        
        if config.train_scale_array is not None:
            img, strong_img, gt, scale = random_scale(img, strong_img, gt, config.train_scale_array)

        strong_img = np.array(strong_img,dtype=np.uint8)
        strong_img,_ = rand_aug(strong_img)
        strong_img = np.array(strong_img,dtype=np.float32)
            
        img = normalize(img, self.img_mean, self.img_std)
        strong_img = normalize(strong_img, self.img_mean, self.img_std)
        ...
        p_img_strong, _ = random_crop_pad_to_shape(strong_img, crop_pos, crop_size, 0)
        ...
        p_img_strong = p_img_strong.transpose(2, 0, 1)

        extra_dict = {}

        return p_img, p_img_strong, p_gt, p_cgt, extra_dict
...
def get_train_loader(engine, dataset, train_source, unsupervised=False, collate_fn=None, sampler_seed = 0):
...
#             train_dataset)
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            train_dataset, shuffle=True, seed = sampler_seed )
...
class CityScape(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.preprocess is not None:
            img, img_strong, gt, edge_gt, extra_dict = self.preprocess(img, gt)
        ...
        output_dict = dict(data=img, data_strong=img_strong, fn=str(item_name),
