...
C.output_path = os.environ['OUTPUT_PATH']
if os.getenv('snapshot_dir'):
    if not os.path.exists(osp.join(os.environ['snapshot_dir'], "snapshot")) :
        os.makedirs(osp.join(os.environ['snapshot_dir'], "snapshot"),exist_ok=True)         
    if not os.path.exists(osp.join(os.environ['snapshot_dir'], "ema_snapshot")) :
        os.makedirs(osp.join(os.environ['snapshot_dir'], "ema_snapshot"),exist_ok=True)         
    C.snapshot_dir = osp.join(os.environ['snapshot_dir'], "snapshot")
    C.ema_snapshot_dir = osp.join(os.environ['snapshot_dir'], "ema_snapshot")   
else:
    C.snapshot_dir = osp.abspath(osp.join(C.log_dir, "snapshot"))    
    
    
exp_time = time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime())
C.log_file = C.log_dir +  '/' + os.path.basename(C.output_path) + '_log_' + exp_time + '.log'
C.link_log_file = C.log_file + '/log_last.log'
C.val_log_file = C.log_dir + '/' + os.path.basename(C.output_path) + '_VALlog_' + exp_time + '.log'
C.link_val_log_file = C.log_dir + '/val_last.log'

"""Data Dir and Weight Dir"""
C.dataset_path = os.path.join(C.volna, 'DATA/city')
C.img_root_folder = C.dataset_path
C.gt_root_folder = C.dataset_path

if os.getenv('pretrained_model') is not None:
    C.pretrained_model = os.environ['pretrained_model']
else : 
