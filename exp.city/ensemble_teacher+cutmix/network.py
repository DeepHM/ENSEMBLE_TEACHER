...
class Network(nn.Module):
    ...
    def forward(self, data, step=1, ensemble='v0'):
        if not self.training:
            if ensemble == 'v0' :
                ...
                pred1 = self.ema_branch1(data)
                return pred1
            elif ensemble == 'v1_2' :
                pred2 = self.ema_branch2(data)
                return pred2
            elif ensemble == 'v2' :
                pred1 = self.branch1(data)
                pred2 = self.branch2(data)
                ...
                pred_s1 = self.branch1(data)
                pred_s2 = self.branch2(data)
                pred_t1 = self.ema_branch1(data)
                pred_t2 = self.ema_branch2(data)
                return ((pred_s1+pred_s2)/2)+((pred_t1+pred_t2)/2)
...
class SingleNetwork(nn.Module):
    def __init__(self, num_classes, criterion, norm_layer, pretrained_model=None):
        ...
        assert ('resnet50' in os.path.basename(pretrained_model)) or ('resnet101' in os.path.basename(pretrained_model)) , \
            'Currently, only resnet50 or resnet101 can be used for pretrained_model.' 
        if (pretrained_model is None) or ('resnet50' in os.path.basename(pretrained_model)) :
            self.backbone = resnet50(pretrained_model, norm_layer=norm_layer,
                          bn_eps=config.bn_eps,
                          bn_momentum=config.bn_momentum,
                          deep_stem=True, stem_width=64)
        elif 'resnet101' in os.path.basename(pretrained_model) :
