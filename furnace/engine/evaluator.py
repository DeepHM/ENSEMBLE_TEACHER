...
class Evaluator(object):
    def __init__(self, dataset, class_num, image_mean, image_std, network,
                 ...
                 ensemble='v0',ema_model_path=None ):
        self.eval_time = 0
        ...
        self.ema_model_path = ema_model_path
...
#                 sorted_models[idx] = m
                print(m)
                num = int(''.join(num for num in m if num.isdigit()))
                model_idx[idx] = num
                sorted_models[idx] = m
        ...
        for model in models:
            ...
            if model is not None:
                ...
                if self.ensemble in ['v1','v1_2','v3','v4'] : 
                    ema_branch1 = copy.deepcopy(self.val_func.branch1)
                    ema_branch2 = copy.deepcopy(self.val_func.branch2)
                    num = model.split('/')[-1]
                    num = int(''.join(n for n in num if n.isdigit()))
                    print(' > Load EMA-Model ! ')
                    print(  ' > ',osp.join(self.ema_model_path,'ema_branch1_epoch{}.pth'.format(num) ))
                    print(  ' > ',osp.join(self.ema_model_path,'ema_branch2_epoch{}.pth'.format(num) ))
                    check_point = torch.load(osp.join(self.ema_model_path,'ema_branch1_epoch{}.pth'.format(num) ))
                    ema_branch1.load_state_dict(check_point['model'])
                    check_point = torch.load(osp.join(self.ema_model_path,'ema_branch2_epoch{}.pth'.format(num) ))
                    ema_branch2.load_state_dict(check_point['model'])
                    self.val_func.ema_branch1 = ema_branch1
                    self.val_func.ema_branch2 = ema_branch2
            ...
            if os.path.exists('log_df.csv') :
                ...
                csv_log = result_line[result_line.find('mean_IU'):].split('\t')
                csv_log = [log_file.split('/')[-1]]+[self.save_path.split('/')[-2]]+[model.split('/')[-1]]+[csv_log[1]]+[csv_log[3]]+[csv_log[5]]+[self.ensemble]
                csv_log = pd.DataFrame([csv_log],columns=['log_file','folder','model_file','mean_IU','mean_IU_no_back','mean_pixel_ACC','Ensemble'])
                log_df = pd.concat([log_df,csv_log]).reset_index(drop=True)
...
#                 score = self.val_func(input_data)
                score = self.val_func(input_data,ensemble=self.ensemble)
                score = score[0]
